...
data_a = data_a.view(np.float32)
data_b = data_b.view(np.float32)
...
if is_debug is True:
    data_a = [0, 0x70000000, 0xffffffff, 0xf0000000, 0xf0000]
    data_b = [0, 0x7fffffff, 0x00000001, 0xf0000000, 0xfffff] 
    data_a = np.array(data_a, np.uint32)
    data_b = np.array(data_b, np.uint32)

a_temp = data_a.view(np.uint32)
b_temp = data_b.view(np.uint32)
a_rm_sign = a_temp & 0x7fffffff
b_rm_sign = b_temp & 0x7fffffff
...
if is_debug_print is True:
    for i in a_temp:
        print(np.binary_repr(i, width=32), end=" ")
    print("")
    for i in b_temp:
        print(np.binary_repr(i, width=32), end=" ")
    print("")
...
for i in range(33):
    ...
    mask_condition =  ab_high_bit == 32 - i
