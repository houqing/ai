...
if len(sys.argv) == 5 and sys.argv[4] in [ 'f16', 'fp16', '16', 'h' ]:
...
def gen_avgs(data, is_need_detail=True):
    data_avg_s = np.mean(data) if len(data) else 0
    data_avg = np.array(data)
    data_avg.fill(data_avg_s)
    ...
    if is_need_detail:
        ...
        data_neg_avg = np.array(data)
        data_neg_avg.fill(data_neg_avg_s)
        return data_avg, data_avg_s, data_pos_avg, data_pos_avg_s, data_neg_avg, data_neg_avg_s
    else:
        return data_avg, data_avg_s, None, None, None, None
...
# set diff param
diff_eps = 1e-7 if is_diff_eps_for_fp16 else 1e-9
...
diff_rel = abs(aa - bb) / (abs(aa) + abs(bb) + diff_eps)
...
diff_info = 'avg_abs='+str(diff_abs_avg_s)+' avg_pos='+str(diff_pos_avg_s)+' avg_neg='+str(diff_neg_avg_s)
diff_rel_info = 'avg='+str(diff_rel_avg_s)+' eps='+str(diff_eps)
...
# calc figure specific data

# output figure
fig_title = f_a_info + '\n' + f_b_info
fig_avg_linewidth = 1
fig_diff_abs_linewidth = 1
fig_markersize_a = 1
fig_markersize_b = 1
fig_markersize = 1
